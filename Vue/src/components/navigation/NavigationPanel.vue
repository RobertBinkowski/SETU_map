<template>
  <div id="navigationDetails">
    ...
    <div>
      <h5>Departure</h5>
      <div :class="navigation.edit === 'departure' ? 'location selected' : 'location'
        ">
        <DetailsComponent v-if="navigation.departure" :location="navigation.departure" @clearLocation="clearDeparture"
          @editLocation="editLocation"></DetailsComponent>
        ...
        <span class="edit">
          <svg @click="setEdit('departure')" xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 512 512">
            <path
    ...
    <div>
      <h5>Destination</h5>
      <div :class="navigation.edit === 'destination' ? 'location selected' : 'location'
        ">
        <DetailsComponent :location="navigation.destination" @clearLocation="clearDestination"
          @editLocation="editLocation"></DetailsComponent>
      ...
      <div class="options">
        <input type="checkbox" name="disabled" class="checkbox" v-model="isChecked" @change="toggleDisabled" /><label
          for="disabled">Wheelchair Accessible</label>
      </div>
    </div>
    <div class="details">
...
export default {
  ...
  methods: {
    navigate() {
      // If no departure/destination is selected, select the closest node
      // else select the location
      if (this.navigation.departure == null) {
        ...
        this.navigation.departure = this.navigation.departure.location;
      }
      if (this.navigation.destination == null) {
        try {
          this.navigation.destination = this.entrance;
        } catch (err) {
          this.navigation.destination = getClosestNode(
            this.locations,
            52.82866813716404,
            -6.936708227680724,
            0
        ...
        this.navigation.destination = this.navigation.destination.location;
      }
